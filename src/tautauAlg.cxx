...
StatusCode tautauAlg::initialize(){
	...
	if ( nt1 ) m_tuple1 = nt1;
     ...
     NTuplePtr nt3(ntupleSvc(),"FILE1/theory");
     if ( nt3 ) m_tuple3 = nt3;
     else {
		m_tuple3 = ntupleSvc()->book ("FILE1/theory", CLID_ColumnWiseTuple, "theory");
		if ( m_tuple3 ){
            m_tuple3->addItem ("pion_px",            pi_px);
            m_tuple3->addItem ("pion_py",            pi_py);
            m_tuple3->addItem ("pion_pz",            pi_pz);
            m_tuple3->addItem ("pion_e",             ma_pi_e);

            m_tuple3->addItem ("gam_px",             gam_px);
            m_tuple3->addItem ("gam_py",             gam_py);
            m_tuple3->addItem ("gam_pz",             gam_pz);
            m_tuple3->addItem ("gam_e",              ma_gam_e);

            m_tuple3->addItem ("tau_px",             tau_px);
            m_tuple3->addItem ("tau_py",             tau_py);
            m_tuple3->addItem ("tau_pz",             tau_pz);
            m_tuple3->addItem ("tau_e",              ma_tau_e);

            m_tuple3->addItem ("nu_px",              nu_px);
            m_tuple3->addItem ("nu_py",              nu_py);
            m_tuple3->addItem ("nu_pz",              nu_pz);
            m_tuple3->addItem ("nu_e",               ma_nu_e);

            m_tuple3->addItem ("_x",                m__x);
            m_tuple3->addItem ("_y",                m__y);
            m_tuple3->addItem ("_t",                m__t);
            m_tuple3->addItem ("_s",                m__s);
            m_tuple3->addItem ("_E",                m__E);
            m_tuple3->addItem ("_z",                m__z);
	...
	if ( nt2 ) m_tuple2 = nt2;
        ...
        //    ma_nu_e = 0.0;
			...
			m_tuple2->addIndexedItem("primaryParticleMC", m_nTrack, ma_primaryParticle);
...
StatusCode tautauAlg::execute() {
  ...
  EvtRecTrackIterator neutral_end   = track_begin + evtRecEvent->totalTracks();
	...
	if (eventHeader->runNumber()<0 && m_testMC == 1){
        ...
        m__x = 2.0*(ma_tau_e*ma_gam_e - tau_px*gam_px 
                - tau_py*gam_py - tau_pz*gam_pz)/pow_tau;
        m__y = 2.0*(ma_tau_e*ma_pi_e - tau_px*pi_px
                - tau_py*pi_py - tau_pz*pi_pz)/pow_tau;	
	        
        double m_t = (ma_pi_e+ma_gam_e)*(ma_pi_e+ma_gam_e) - (pi_px+gam_px)*(pi_px+gam_px) 
                   - (pi_py+gam_py)*(pi_py+gam_py) - (pi_pz+gam_pz)*(pi_pz+gam_pz);
        m__t = sqrt(m_t);

        m__s = ma_pi_e*ma_pi_e - pi_px*pi_px -pi_py*pi_py -pi_pz*pi_pz
              + 2.0*(ma_pi_e*ma_gam_e -pi_px*gam_px -pi_py*gam_py -pi_pz*gam_pz)
              + ma_gam_e*ma_gam_e - gam_px*gam_px -gam_py*gam_py -gam_pz*gam_pz;
		m__z = m__x + m__y - 1.0;
	...
	// reconstruct 2 good charged tracks, and don't limit the number of good photons
	...
	if(!(nGood == 2 && charge ==0)) return StatusCode::SUCCESS; 
	Ncut[1]++;

	//2020.03.14, make sure the first track is positive, the second one is negetive;
	...
	if(igamma > 100) return StatusCode::SUCCESS;
	Ncut[2]++;
	...
	Hep3Vector P3_1;
	Hep3Vector P3_2;
	RecMdcKalTrack::PidType pidtype[5] = {RecMdcKalTrack::electron,
		RecMdcKalTrack::muon, RecMdcKalTrack::pion,
...
Ncut[3]++;
      ...
      m_nTrk_EMC = nchrgtrk;
      if (nchrgtrk != 2) return StatusCode::SUCCESS;
      Ncut[4]++;
      if (itrk[0]==itrk[1]) return StatusCode::SUCCESS;
      Ncut[5]++;

      //2020.03.15, to make sure the opposite charge of two tracks
      if (!(m_charge_1 * m_charge_2 <0)) return StatusCode::SUCCESS;
      Ncut[6]++;
	...
	m_m2Trks[i] = pkal_Trks[i].m2();
	  ...
	  if (m_phi_diff < 0) m_phi_diff = m_phi_diff + twopi;
...
m_miss_m2_ee   = (m_ecms-P4_ee.e())*(m_ecms-P4_ee.e())-((p_all-P4_ee).rho())*((p_all-P4_ee).rho());
m_miss_m2_emu  = (m_ecms-P4_emu.e())*(m_ecms-P4_emu.e())-((p_all-P4_emu).rho())*((p_all-P4_emu).rho());
m_miss_m2_mue  = (m_ecms-P4_mue.e())*(m_ecms-P4_mue.e())-((p_all-P4_mue).rho())*((p_all-P4_mue).rho());
m_miss_m2_epi  = (m_ecms-P4_epi.e())*(m_ecms-P4_epi.e())-((p_all-P4_epi).rho())*((p_all-P4_epi).rho());
m_miss_m2_pie  = (m_ecms-P4_pie.e())*(m_ecms-P4_pie.e())-((p_all-P4_pie).rho())*((p_all-P4_pie).rho());
m_miss_m2_ek   = (m_ecms-P4_ek.e())*(m_ecms-P4_ek.e())-((p_all-P4_ek).rho())*((p_all-P4_ek).rho());
m_miss_m2_ke   = (m_ecms-P4_ke.e())*(m_ecms-P4_ke.e())-((p_all-P4_ke).rho())*((p_all-P4_ke).rho());
m_miss_m2_mumu = (m_ecms-P4_mumu.e())*(m_ecms-P4_mumu.e())-((p_all-P4_mumu).rho())*((p_all-P4_mumu).rho());
m_miss_m2_mupi = (m_ecms-P4_mupi.e())*(m_ecms-P4_mupi.e())-((p_all-P4_mupi).rho())*((p_all-P4_mupi).rho());
m_miss_m2_pimu = (m_ecms-P4_pimu.e())*(m_ecms-P4_pimu.e())-((p_all-P4_pimu).rho())*((p_all-P4_pimu).rho());
m_miss_m2_pipi = (m_ecms-P4_pipi.e())*(m_ecms-P4_pipi.e())-((p_all-P4_pipi).rho())*((p_all-P4_pipi).rho());
m_miss_m2_muk  = (m_ecms-P4_muk.e())*(m_ecms-P4_muk.e())-((p_all-P4_muk).rho())*((p_all-P4_muk).rho());
m_miss_m2_kmu  = (m_ecms-P4_kmu.e())*(m_ecms-P4_kmu.e())-((p_all-P4_kmu).rho())*((p_all-P4_kmu).rho());
m_miss_m2_pik  = (m_ecms-P4_pik.e())*(m_ecms-P4_pik.e())-((p_all-P4_pik).rho())*((p_all-P4_pik).rho());
m_miss_m2_kpi  = (m_ecms-P4_kpi.e())*(m_ecms-P4_kpi.e())-((p_all-P4_kpi).rho())*((p_all-P4_kpi).rho());
m_miss_m2_kk   = (m_ecms-P4_kk.e())*(m_ecms-P4_kk.e())-((p_all-P4_kk).rho())*((p_all-P4_kk).rho());
...
m_ee_PTEM   = (P3_trk1[0] + P3_trk2[0]).perp() / (m_ecms - (P3_trk1[0] + P3_trk2[0]).r());
m_emu_PTEM  = (P3_trk1[0] + P3_trk2[1]).perp() / (m_ecms - (P3_trk1[0] + P3_trk2[1]).r());
m_mue_PTEM  = (P3_trk1[1] + P3_trk2[0]).perp() / (m_ecms - (P3_trk1[1] + P3_trk2[0]).r());
m_epi_PTEM  = (P3_trk1[0] + P3_trk2[2]).perp() / (m_ecms - (P3_trk1[0] + P3_trk2[2]).r());
m_pie_PTEM  = (P3_trk1[2] + P3_trk2[0]).perp() / (m_ecms - (P3_trk1[2] + P3_trk2[0]).r());
m_ek_PTEM   = (P3_trk1[0] + P3_trk2[3]).perp() / (m_ecms - (P3_trk1[0] + P3_trk2[3]).r());
m_ke_PTEM   = (P3_trk1[3] + P3_trk2[0]).perp() / (m_ecms - (P3_trk1[3] + P3_trk2[0]).r());
m_mumu_PTEM = (P3_trk1[1] + P3_trk2[1]).perp() / (m_ecms - (P3_trk1[1] + P3_trk2[1]).r());
m_mupi_PTEM = (P3_trk1[1] + P3_trk2[2]).perp() / (m_ecms - (P3_trk1[1] + P3_trk2[2]).r());
m_pimu_PTEM = (P3_trk1[2] + P3_trk2[1]).perp() / (m_ecms - (P3_trk1[2] + P3_trk2[1]).r());
m_pipi_PTEM = (P3_trk1[2] + P3_trk2[2]).perp() / (m_ecms - (P3_trk1[2] + P3_trk2[2]).r());
m_muk_PTEM  = (P3_trk1[1] + P3_trk2[3]).perp() / (m_ecms - (P3_trk1[1] + P3_trk2[3]).r());
m_kmu_PTEM  = (P3_trk1[3] + P3_trk2[1]).perp() / (m_ecms - (P3_trk1[3] + P3_trk2[1]).r());
m_pik_PTEM  = (P3_trk1[2] + P3_trk2[3]).perp() / (m_ecms - (P3_trk1[2] + P3_trk2[3]).r());
m_kpi_PTEM  = (P3_trk1[3] + P3_trk2[2]).perp() / (m_ecms - (P3_trk1[3] + P3_trk2[2]).r());
m_kk_PTEM   = (P3_trk1[3] + P3_trk2[3]).perp() / (m_ecms - (P3_trk1[3] + P3_trk2[3]).r());
...
if (evtfg == 1) {//pi0
	...
	m_miss_m2_erho_11 = (m_ecms-P4_erho_11.e())*(m_ecms-P4_erho_11.e())-((p_all-P4_erho_11).rho())*((p_all-P4_erho_11).rho());
	m_miss_m2_erho_12 = (m_ecms-P4_erho_12.e())*(m_ecms-P4_erho_12.e())-((p_all-P4_erho_12).rho())*((p_all-P4_erho_12).rho());
	m_miss_m2_murho_11 = (m_ecms-P4_murho_11.e())*(m_ecms-P4_murho_11.e())-((p_all-P4_murho_11).rho())*((p_all-P4_murho_11).rho());
	m_miss_m2_murho_12 = (m_ecms-P4_murho_12.e())*(m_ecms-P4_murho_12.e())-((p_all-P4_murho_12).rho())*((p_all-P4_murho_12).rho());
...
if (evtfg == 2) {//rho
		...
		for (int j=i+1; j<nGam-2; j++) {
			for (int k=j+1; k<nGam-1; k++) {
				for (int m=k+1; m<nGam; m++) {
					...
					m2g_1 = p2g_1.m();
					m2g_2 = p2g_2.m();
					dltm2g = sqrt((m2g_1-mpi0)*(m2g_1-mpi0) + (m2g_2-mpi0)*(m2g_2-mpi0));
					if (delmpi0 > dltm2g) {
						delmpi0 = dltm2g;
						g1 = i;
						g2 = m;
	...
	m_miss_m2_rhorho_11 = (m_ecms-P4_rhorho_11.e())*(m_ecms-P4_rhorho_11.e())-((p_all-P4_rhorho_11).rho())*((p_all-P4_rhorho_11).rho());
	m_miss_m2_rhorho_12 = (m_ecms-P4_rhorho_12.e())*(m_ecms-P4_rhorho_12.e())-((p_all-P4_rhorho_12).rho())*((p_all-P4_rhorho_12).rho());
...
if (evtfg!=1 && evtfg!=2) {
  ...
  for(int k=0; k < iGood.size(); k++){
    ...
    prob_ratio_electron= prob_electron / (prob_pion + prob_electron + prob_kaon); 
		...
		if(prob_electron > 0.001 &&prob_electron > prob_pion && prob_electron > prob_kaon){
  ...
  Ncut[4]++;
	if(!((elp_id >-1 && mum_id>-1) &&(elp_id + mum_id) == 1))  return StatusCode::SUCCESS;
  Ncut[5]++;
*/
...
/*  CDPhotonList      photonList(neutral_begin, neutral_end, photonSelector);
  ...
  //2020.3.9, save gamma -> e+ e-
	RecMdcKalTrack *elpTrk = (*(evtRecTrkCol->begin()+iGood[elp_id]))->mdcKalTrack();
	RecMdcKalTrack::setPidType(RecMdcKalTrack::electron); 
	RecMdcKalTrack *mumTrk = (*(evtRecTrkCol->begin()+iGood[mum_id]))->mdcKalTrack();
	//RecMdcKalTrack::setPidType(RecMdcKalTrack::muon); 
	RecMdcKalTrack::setPidType(RecMdcKalTrack::electron);//to see if there is electron misidentified as muon
  checkGammConversion(xorigin, elpTrk, mumTrk, m_gamcon);

  //2020.3.9, correction for electron due to bremsstrahlung, add the energy of photon from electron back to electron;
	...
	if (m_appFSRCorrection ) {
		...
		setV4(m_gam_fromelp,    p4FSRCor[0]);// the total photons from electron
		setV4(m_elp_corfsr,     p4elp_corfsr);// the electron after adding the photons from elctron 
  } 
...
// -------------------------------DDbar-----------------------------------
	...
	{
		m_pdgid[m_idxmc]=22222222; // 22222222 is the default PDG code of ISR photons in TopoAna.
		m_rec_pdgid[m_idxmc]=22222222; // 22222222 is the default PDG code of ISR photons in TopoAna.
	...
	else if(incPdcy==true&&(*iter_mc)->particleProperty()==11&&((*iter_mc)->mother()).trackIndex()==(*iter_mc)->trackIndex())
	{
		continue; // This statement is used to remove the redundant electron following Y(4260). 
	}
	if(!incPdcy) continue;
	...
	if((*iter_mc)->particleProperty()==incpdgid0){
		 ...
		 m_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-rootIndex+ngamma;
		 m_rec_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-rootIndex+ngamma;
	}
	else {
		m_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-rootIndex-1+ngamma;
		m_rec_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-rootIndex-1+ngamma;
	}
	m_idxmc++;
	m_rec_idxmc++;
}
...
if(incPdcy==false)
{
	...
	for (;iter_mc!=mcParticleCol->end();iter_mc++)
	{
		...
		else if((*iter_mc)->particleProperty()==incpdgid1||(*iter_mc)->particleProperty()==incpdgid2)
		{
			incPdcy=true;
			rootIndex=(*iter_mc)->trackIndex();
		}
		...
		if(!incPdcy||(*iter_mc)->particleProperty()==incpdgid1||(*iter_mc)->particleProperty()==incpdgid2) continue;
		m_pdgid[m_idxmc]=(*iter_mc)->particleProperty();
		m_rec_pdgid[m_idxmc]=(*iter_mc)->particleProperty();
		if(((*iter_mc)->mother()).particleProperty()==incpdgid1||((*iter_mc)->mother()).particleProperty()==incpdgid2) {
		m_motheridx[m_idxmc]=-1;
		m_rec_motheridx[m_idxmc]=-1;
		}
		else {
...
// ---------------QED (bhabha, gamgam, mumu, tautau, qqbar, qqbar2)------------------
if(incPdcy==false)
{
	iter_mc=mcParticleCol->begin();
	...
	m_idxmc=0;
	m_rec_idxmc=0;
	int idxOfTRE=-1; // TRE is short for The Redundant Electron. Statements related to the variable is used to remove the redundant electron in the tautau sample.
	int nOfTRQG=0; // TRQG is short for The Redundant quarks and gluons. Statements related to the variable is used to remove the d, dbar, u, ubar, s, sbar, c, cbar, and g in the qqbar and qqbar2 samples.
	for(;iter_mc!=mcParticleCol->end();iter_mc++)
	{
		//if(!(*iter_mc)->decayFromGenerator()) continue;
		// std::cout<<"idxmc="<<m_idxmc<<"\t"<<"pdg\t"<<"motheridx="<<((*iter_mc)->mother()).trackIndex()<<std::endl;
		if(pdgid==23&&(*iter_mc)->particleProperty()==11&&((*iter_mc)->mother()).trackIndex()==m_idxmc)
		{
			idxOfTRE=m_idxmc; 
			continue;
		}
		if(abs((*iter_mc)->particleProperty())==1||abs((*iter_mc)->particleProperty())==2||abs((*iter_mc)->particleProperty())==3||abs((*iter_mc)->particleProperty())==4||abs((*iter_mc)->particleProperty())==21)
		{
			nOfTRQG++; 
			continue;
		}
		...
		m_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-1-nOfTRQG;
		m_rec_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-1-nOfTRQG;
		}
		else {
			m_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-nOfTRQG;
			m_rec_motheridx[m_idxmc]=((*iter_mc)->mother()).trackIndex()-nOfTRQG;
		}
		m_idxmc++;
		m_rec_idxmc++;
	}
...
bool tautauAlg::correctLeptonMomentumWithFSRPhoton( HepLorentzVector p4lepp, std::vector<HepLorentzVector>& p4FSRCor){
	p4FSRCor.clear();
	...
	for(int i = evtRecEvent->totalCharged(); i< evtRecEvent->totalTracks(); i++) {
		...
		dthelepp = fabs(fmod(dthelepp+CLHEP::twopi+CLHEP::twopi+pi, CLHEP::twopi) - CLHEP::pi );
